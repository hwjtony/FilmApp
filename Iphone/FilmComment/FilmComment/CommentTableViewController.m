...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cell.imageView.image = comment.img;
	cell.textLabel.text = comment.name;
    cell.detailTextLabel.text = comment.title;
	return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [vc setValue:comment.cid forKey:@"cid"];
    //[vc setValue:comment.time forKey:@"time"];
    NSLog(@"table cid%@",comment.cid);
    //[vc setValue:comment.title forKey:@"commentTitle"];
    //[vc setValue:comment.name forKey:@"name"];
    //[vc setValue:comment.content forKey:@"content"];
    NSLog(@"table comment content %@",comment.content);
...
forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.typeListContent removeObjectAtIndex:row];
    [tableView deleteRowsAtIndexPaths:[NSArray arrayWithObject:indexPath]
                     withRowAnimation:UITableViewRowAnimationAutomatic];
}

-(void)SearchData:(int)offset{
    NSLog(@"comment count%d",offset);
    NSString *urlString = [NSString stringWithFormat:@"http://192.168.1.11/Movie/index.php?r=site/CommentSearch&offset=%d&fid=%@",offset,_data];
    NSLog(@"%@",_data);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConn {
    ...
    for (NSMutableDictionary *picture in root) {
        ...
        NSString *content = [picture objectForKey:@"content"];
        NSString *cid = [picture objectForKey:@"cid"];
        NSString *time = [picture objectForKey:@"time"];
        ...
        [self.typeListContent addObject:[Comment commentWithName:name content:content title:title cid:cid time:time]];
        NSLog(@"cid %@",cid);
    }
