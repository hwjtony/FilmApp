...
- (id)initWithFrame:(CGRect)frame title:(NSString *)title data:(NSString *)data {
	if ((self = [super initWithFrame:frame])) {
		...
		[wv loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat: @"http://192.168.3.12/index.php?r=site/addCommentView&fid=%@",data]]]];
        ...
        [iv addSubview:textView];
		
		[[NSBundle mainBundle] loadNibNamed:@"UAExampleView" owner:self options:nil];
		
		NSArray *contentArray = [NSArray arrayWithObjects:wv, nil];
